...
Vagrant.configure("2") do |config|
  ...
  config.vm.provision "shell", inline: <<-SHELL
    apt-get update
    apt-get install -y build-essential gcc-multilib emacs htop gdb valgrind
    ...
    for i in `ls /home`; do curl http://www.cse.psu.edu/~tuz68/.vimrc 2>/dev/null >/home/${i}/.vimrc; chown ${i}:${i} /home/${i}/.vimrc; done
  SHELL
end
